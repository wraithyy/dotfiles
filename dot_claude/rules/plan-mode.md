...
Before spawning any Explore agents, conduct grill-me interview:

- Ask one question at a time
- For each question, provide your recommended answer
- If question can be answered by reading codebase, explore instead — don't ask
- Continue until user confirms shared understanding
- Only then proceed to Phase 1 (Explore agents)

## Plan File: Atomic Task Schema

Final plan file MUST break work into atomic tasks. Each task = one file or one concern, independently verifiable.
...
Assign agents from `rules/agents.md`. Default Haiku for mechanical tasks, Sonnet for implementation, Opus only for architecture.

Tasks with no shared dependencies can be dispatched in parallel via single `Agent` message with multiple tool calls.
...
4. **Tasks** — numbered atomic blocks using schema above
